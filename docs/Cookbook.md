...
```bash
$ docker create --name wwwdata -v /usr/share/nginx/www ahmaddanni/docker-nginx-fpm-caches-wordpress
```
...
```bash
$ docker run --rm --volumes-from wwwdata -v $(pwd):/backup ahmaddanni/docker-nginx-fpm-caches-wordpress tar -cvz  -f /backup/wwwdata.tar.gz /usr/share/nginx/www

```
#### Import into a data volume container:

```bash
$ docker run --rm --volumes-from wwwdata2 -v $(pwd):/new-data ahmaddanni/docker-nginx-fpm-caches-wordpress bash -c 'cd / && tar xzvf /new-data/wwwdata.tar.gz'
```

to verify the content:

```bash
$ docker run --rm --volumes-from wwwdata2 -v $(pwd):/new-data -it ahmaddanni/docker-nginx-fpm-caches-wordpress bash
```
...
$ docker run --rm  --link mysqlserver:db -v $(pwd):/dbbackup -it ahmaddanni/docker-nginx-fpm-caches-wordpress bash
...
```bash
$ docker run --rm  --link mysqlserver:db -it ahmaddanni/docker-nginx-fpm-caches-wordpress bash
